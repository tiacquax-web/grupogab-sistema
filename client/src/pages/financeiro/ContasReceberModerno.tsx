...
function StatusBadge({ status }: { status: string }) {
  ...
  const variant = variants[status] || variants.a_receber;
...
export default function ContasReceberModerno() {
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<string>("all");

  // Queries
  const { data: summary } = trpc.financial.summaryReceivable.useQuery();
  const { data: accounts = [], isLoading } = trpc.financial.listReceivable.useQuery({
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <SummaryCard
          ...
          trend={`${summary?.receivedCount || 0} recebimentos`}
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant={statusFilter === "a_receber" ? "default" : "outline"}
                      ...
                      <div className="text-right">
                        ...
                        {account.receivedDate && (
                          <p className="text-sm text-blue-600">
                            Recebido em {new Date(account.receivedDate).toLocaleDateString('pt-BR')}
                          </p>
                        )}
                      </div>
